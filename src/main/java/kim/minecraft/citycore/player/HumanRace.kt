...
@Serializable
data class HumanRace(@Serializable(with = UUIDAsStringSerializer::class) val player: UUID, var name: String,@Serializable(with = UUIDAsStringSerializer::class) var currentCountry: UUID?,@Serializable(with = UUIDAsStringSerializer::class) var currentParty: UUID?, var alive: Boolean, override val wallet: Wallet, override val permissions: MutableList<String>) : WalletHolder, CountryOwner, CompanyOwner, PermissionHolder {

    @Serializable(with = UUIDAsStringSerializer::class) override val uniqueID: UUID = UUID.randomUUID()
